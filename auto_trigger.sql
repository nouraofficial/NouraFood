...
create policy profiles_owner_select on public.profiles for select to authenticated using(auth.uid()=id or exists(select 1 from public.profiles p where p.id=auth.uid() and p.role in ('admin','super_admin')));
...
create policy vendors_owner_insert on public.vendors for insert to authenticated with check(auth.uid()=auth_user_id);
drop policy if exists vendors_owner_update on public.vendors;
create policy vendors_owner_update on public.vendors for update to authenticated using(auth.uid()=auth_user_id or exists(select 1 from public.profiles p where p.id=auth.uid() and p.role in ('admin','super_admin'))) with check(auth.uid()=auth_user_id or exists(select 1 from public.profiles p where p.id=auth.uid() and p.role in ('admin','super_admin')));

drop policy if exists restaurants_public_select on public.restaurants;
create policy restaurants_public_select on public.restaurants for select to anon,authenticated using(coalesce(verified,false)=true or exists(select 1 from public.vendors v where v.id=vendor_id and v.auth_user_id=auth.uid()) or exists(select 1 from public.profiles p where p.id=auth.uid() and p.role in ('admin','super_admin')));
drop policy if exists restaurants_owner_update on public.restaurants;
create policy restaurants_owner_update on public.restaurants for update to authenticated using(exists(select 1 from public.vendors v where v.id=vendor_id and v.auth_user_id=auth.uid()) or exists(select 1 from public.profiles p where p.id=auth.uid() and p.role in ('admin','super_admin'))) with check(true);

drop policy if exists menu_public_select on public.menu_items;
create policy menu_public_select on public.menu_items for select to anon,authenticated using(exists(select 1 from public.restaurants r where r.id=restaurant_id and (r.verified=true or exists(select 1 from public.vendors v where v.id=r.vendor_id and v.auth_user_id=auth.uid()) or exists(select 1 from public.profiles p where p.id=auth.uid() and p.role in ('admin','super_admin')))));
drop policy if exists menu_owner_write on public.menu_items;
create policy menu_owner_write on public.menu_items for all to authenticated using(exists(select 1 from public.restaurants r join public.vendors v on v.id=r.vendor_id where r.id=restaurant_id and (v.auth_user_id=auth.uid() or exists(select 1 from public.profiles p where p.id=auth.uid() and p.role in ('admin','super_admin'))))) with check(exists(select 1 from public.restaurants r join public.vendors v on v.id=r.vendor_id where r.id=restaurant_id and (v.auth_user_id=auth.uid() or exists(select 1 from public.profiles p where p.id=auth.uid() and p.role in ('admin','super_admin')))));
